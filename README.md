...
- QTM defines the following global variables before executing PHP:
    ...
    - **$batch_processing_info** provides info if QTM is running in batch processing mode. Values is either "batch" when batch is running or empty for a single session analysis (> QTM 2026.1). 
...
Manager\Packages\` or `C:\Program Files\Qualisys\Qualisys Track
Manager\Packages\`).
...
From the wizard pane, the user presses `Ctrl + Left-Click` to select multiple session folders.  
...
>Notes:
>- The analysis will run on all of the selected folders, skipping any that do not include the chosen analysis name.
>- Warning and Error popup windows generated by QTM are replaced with messages in the QTM Messages window, eliminating the need for user intervention.
